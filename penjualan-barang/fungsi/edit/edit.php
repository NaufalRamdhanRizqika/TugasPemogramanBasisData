...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['barang'])) {
        ...
        $tgl_update = date("Y-m-d H:i:s"); // Format tanggal untuk database

        // Query update
        $data = [$kategori, $nama, $merk, $beli, $jual, $satuan, $stok, $tgl_update, $id];
    ...
    if (!empty($_GET['gambar'])) {
        $id = htmlentities($_POST['id']);
        set_time_limit(0);
        $allowedImageType = ["image/gif", "image/jpeg", "image/pjpeg", "image/png", "image/x-png", 'image/webp'];
        ...
        if (!in_array($filetype, $allowedImageType)) {
            ...
            echo '<script>alert("WARNING !!! File size should not exceed 4 MB");window.location="../../index.php?page=user"</script>';
            exit;
        } else {
            $dir = '../../assets/img/user/';
            ...
            if (move_uploaded_file($tmp_name, $dir . $name)) {
                ...
                $data = [$name, $id];
                $sql = 'UPDATE pegawai SET gambar=? WHERE id_pegawai=?';
                $row = $config->prepare($sql);
                $row->execute($data);
    ...
    if (!empty($_GET['profil'])) {
        ...
        $sql = 'UPDATE pegawai SET nm_pegawai=?, alamat_pegawai=?, telepon=?, email=?, NIK=? WHERE id_pegawai=?';
        $row = $config->prepare($sql);
        $row->execute($data);
        echo '<script>window.location="../../index.php?page=user&success=edit-data"</script>';
    }
    ...
    if (!empty($_GET['pass'])) {
        ...
        $data = [$user, $pass, $id];
        $sql = 'UPDATE login SET user=?, pass=md5(?) WHERE id_pegawai=?';
        $row = $config->prepare($sql);
        $row->execute($data);
        echo '<script>window.location="../../index.php?page=user&success=edit-data"</script>';
    }
    ...
    if (!empty($_GET['cari_barang'])) {
        ...
        if ($cari != '') {
            $sql = "SELECT barang.*, kategori.id_kategori, kategori.nama_kategori FROM barang INNER JOIN kategori ON barang.id_kategori = kategori.id_kategori WHERE barang.id_barang LIKE ? OR barang.nama_barang LIKE ? OR barang.merk LIKE ?";
            $row = $config->prepare($sql);
            ...
            <table class="table table-stripped" width="100%" id="example2">
                ...
                <?php foreach ($hasil1 as $hasil) { ?>
                    <tr>
                        ...
                        <td>
                            <a href="fungsi/tambah/tambah.php?jual=jual&id=<?php echo $hasil['id_barang']; ?>&id_kasir=<?php echo $_SESSION['admin']['id_pegawai']; ?>" class="btn btn-success">
