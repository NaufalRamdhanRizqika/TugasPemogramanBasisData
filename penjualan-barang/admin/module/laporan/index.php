...
<div class="row">
	<div class="col-md-12">
		<?php
		$bulan = isset($_REQUEST['bln']) ? $_REQUEST['bln'] : date('m');
		$tahun = isset($_REQUEST['thn']) ? $_REQUEST['thn'] : date('Y');
		$hari = isset($_REQUEST['hari']) ? $_REQUEST['hari'] : date('Y-m-d');
		?>
		<h4>
			<?php if (!empty($_GET['cari'])) { ?>
				Data Laporan Penjualan <?= $bulan_tes[$bulan]; ?> 	<?= $tahun; ?>
			<?php } elseif (!empty($_GET['hari'])) { ?>
				Data Laporan Penjualan <?= $hari; ?>
			<?php } else { ?>
					...
					<table class="table table-bordered w-100 table-sm" id="example1">
						...
						<tfoot>
							<tr>
								...
								<th>Rp<?php echo number_format($modal, 0, '', '.'); ?></th>
								<th>Rp<?php echo number_format($bayar, 0, '', '.'); ?></th>
